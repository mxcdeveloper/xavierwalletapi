...
describe('Asset', () => {
    ...
    describe('core functionality', () => {
        ...
        it('should switch between storages in different networks', (done) => {
            ...
            Xavier.config.set(XavierAPI.TESTNET_CONFIG);
            Asset.getKnownAssetsList().then((testnetInitialList) => {
                return Asset.get(defaultProps1).then(() => {
                    ...
                    Xavier.config.set(XavierAPI.MAINNET_CONFIG);
        ...
        it('should return a stored asset by its ID', (done) => {
            ...
            const w = Asset.get(Xavier.constants.XAVIER).then((xavier) => {
                expect(xavier.id).to.equal(Xavier.constants.XAVIER);
            });
        ...
        it('should reset cache', (done) => {
            Promise.all([
                ...
                return Asset.getKnownAssetsList().then((list) => {
                    // Only default Xavier asset remains
        ...
        it('should return an object with all known assets', (done) => {
            Promise.all([
                ...
                return Asset.getKnownAssets().then((assets) => {
                    ...
                    expect(keys).to.have.members([Xavier.constants.XAVIER, defaultProps1.id, defaultProps2.id]);
                });
            }).then(() => done());
        });
        ...
        it('should return an array with all known assets', (done) => {
            Promise.all([
                ...
                return Asset.getKnownAssetsList().then((assets) => {
                    ...
                    expect(list).to.have.members([Xavier.constants.XAVIER, defaultProps1.id, defaultProps2.id]);
                });
            }).then(() => done());
        });
