...
describe('Transactions', () => {
    ...
    beforeEach(() => {
        ...
        Transactions = Xavier.Transactions;
        byteProcessors = Xavier.byteProcessors;
        ...
        tempSignDataMethod = Xavier.crypto.buildTransactionSignature;
        Xavier.crypto.buildTransactionSignature = deterministicSign;

    });

    afterEach(() => {
        Xavier.crypto.buildTransactionSignature = tempSignDataMethod;
    });
    ...
    describe('signing transactions', () => {
        ...
        beforeEach(() => {
            ...
            transferData = {
                ...
                amount: 1000,
                feeAssetId: Xavier.constants.XAVIER,
                fee: 100000,
                attachment: '',
            ...
            massTransferEmptyData = {
                senderPublicKey: keys.publicKey,
                assetId: Xavier.constants.XAVIER,
                transfers: [],
            ...
            orderData = {
                ...
                amountAsset: Xavier.constants.XAVIER,
        ...
        it('should sign Issue transaction', (done) => {
            ...
            const api = issueTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.ISSUE_TX_NAME, expectedSignature);
            });
        ...
        it('should sign Transfer transaction', (done) => {
            ...
            const api = transferTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.TRANSFER_TX_NAME, expectedSignature);
        ...
        it('should sign Transfer transaction with alias', (done) => {
            ...
            const api = transferTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.TRANSFER_TX_NAME, expectedSignature);
                expect(preparedData.recipient).to.equal('alias:' + String.fromCharCode(Xavier.config.getNetworkByte()) + ':' + data.recipient);
            });
        ...
        it('should sign Reissue transaction', (done) => {
            ...
            const api = reissueTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.REISSUE_TX_NAME, expectedSignature);
            });
        ...
        it('should sign Burn transaction', (done) => {
            ...
            const api = burnTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.BURN_TX_NAME, expectedSignature);
            });
        ...
        it('should sign Lease transaction', (done) => {
            ...
            const api = leaseTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.LEASE_TX_NAME, expectedSignature);
                expect(preparedData.recipient).to.equal('address:' + data.recipient);
            });
        ...
        it('should sign Cancel Leasing transaction', (done) => {
            ...
            const api = cancelLeasingTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.CANCEL_LEASING_TX_NAME, expectedSignature);
            });
        ...
        it('should sign Create Alias transaction', (done) => {
            ...
            const api = createAliasTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.CREATE_ALIAS_TX_NAME, expectedSignature);
            });
        ...
        it('should sign empty Mass Transfer transaction', (done) => {
            ...
            const api = massTransferTransaction.prepareForAPI(keys.privateKey).then((preparedData) => {
                checkBasicCases(preparedData, data, Xavier.constants.MASS_TRANSFER_TX_NAME, expectedProofs, massTransferAdditionialKeys);
            });
