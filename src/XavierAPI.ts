import { IHash, IXavierConfig } from '../interfaces';
...
class XavierAPI implements IXavierAPI {
    ...
    constructor(initialConfiguration) {
        ...
        if (this instanceof XavierAPI) {

            this.config.clear();
            this.config.set(initialConfiguration);

            if (XavierAPI._instance === null) {
                XavierAPI._instance = this;
            } else {
                return XavierAPI._instance;
            }

        } else {

            return new XavierAPI(initialConfiguration);
...
export function create(config: IXavierConfig): IXavierAPI {
    return new XavierAPI(config);
}

export const MAINNET_CONFIG: IXavierConfig = constants.DEFAULT_MAINNET_CONFIG;
export const TESTNET_CONFIG: IXavierConfig = constants.DEFAULT_TESTNET_CONFIG;
