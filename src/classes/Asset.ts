...
import { XAVIER_PROPS } from '../constants';
...
function getAssetProps(id: string): Promise<IAssetObject> {
    if (id === XAVIER_PROPS.id) {
        return Promise.resolve(XAVIER_PROPS);
    }
...
export default class Asset implements IAsset {
    ...
    private static _storage = getStorage((set) => {
        return Asset._factory(XAVIER_PROPS.id).then((xavierAsset) => {
            set(xavierAsset.id, xavierAsset);
        });
    });
