...
// NOTE : Xavier asset ID in blockchain transactions equals to an empty string
function blockchainifyAssetId(assetId) {
    if (!assetId) throw new Error('Asset ID should not be empty');
    return assetId === constants.XAVIER ? '' : assetId;
}
...
export class AssetId extends ByteProcessor {
    public process(value: string) {
        ...
        // We must pass bytes of `[0]` for Xavier asset ID and bytes of `[1] + assetId` for other asset IDs
