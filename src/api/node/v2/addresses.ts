...
function getBalances(address, options): Promise<Money[]> {
    ...
    // TODO : avoid unnecessary requests if the asset option is provided and Xavier or any assets are not in it

    const xavierBalance = v1Addresses.balanceDetails(address).then((data) => {
        return Money.fromCoins(String(data.available), constants.XAVIER).then((amount) => [amount]);
    });
    ...
    return Promise.all([xavierBalance, assetBalances])
        .then(([xavier, assets]) => {
            const array = [...xavier, ...assets];
            if (options.assets) {
...
export default {
    ...
    get(address) {
        ...
        const balanceDetails = v1Addresses.balanceDetails(address).then((data) => {
            return schemas.detailedXavierBalanceSchema.parse(data);
        });
        ...
        return Promise.all([
            balanceDetails,
            aliasesByAddress
        ]).then(([xavierBalance, aliases]) => {
            return { xavierBalance, aliases };
        });
