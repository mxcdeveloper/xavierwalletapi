...
const getTxCommonFields = (typeName, xavierFeeOnly) => ({
    transactionType: {
        type: StringPart,
...
const getTxAssetId = (path?) => ({
    ...
    required: true,
    path: path,
    parseValue: (d) => d ? d : constants.XAVIER
});
...
export const exchangeTransactionSchema = new Schema({
    ...
    content: {
        ...
        buyMatcherFee: getTxXavierFee(),
        sellOrder: getTxOrder('order2'),
        sellMatcherFee: getTxXavierFee()
    }
});
